...
ggplot_add.geom_outline <- function(object, plot, object_name) {
  ...
  if (out_pos == "bottom" || is.na(grps)) {
    ...
    plt_dat <- dplyr::mutate(
      plt_dat,
      .GROUPS = ifelse(!!sym(grps) %in% out_pos, as.character(!!sym(grps)), ignore_grp),
      .GROUPS = factor(.GROUPS, c(ignore_grp, out_pos))
    )
  }
